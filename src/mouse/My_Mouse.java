...
public class My_Mouse implements Mouse {
	...
	/*	
		...
		//если нет стенки
		if(!(map.containsKey(s1)) && (oldDirection == Direction.Up)){
				n++;
				oldDirection = Direction.Right;
				return Direction.Right;
		}
		if(!(map.containsKey(s2))&& (oldDirection == Direction.Down)){
				n--;
				oldDirection = Direction.Left;
				return Direction.Left;
		}
		if(!(map.containsKey(s3)) && (oldDirection == Direction.Left)){
				m--;
				oldDirection = Direction.Up;
				return Direction.Up;
		}
		if(!(map.containsKey(s4)) && (oldDirection == Direction.Right)){
				m++;
				oldDirection = Direction.Down;
				return Direction.Down;
		}
